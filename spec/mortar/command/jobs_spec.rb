    ...
    context("run") do
      it "handles singlejobcluster parameter" do
        with_git_initialized_project do |p|
          ...
          mock(Mortar::Auth.api).post_job_new_cluster("myproject", "my_script", is_a(String), "0.9", cluster_size, 
...
STDOUT
      ...
      it "runs a control script using new full-path syntax" do
        with_git_initialized_project do |p|
          ...
          mock(Mortar::Auth.api).post_job_existing_cluster("myproject", "my_script", is_a(String), "0.9", cluster_id,
              :parameters => [],
...
STDOUT
      ...
      it "runs a job on an existing cluster" do
        with_git_initialized_project do |p|
          ...
          mock(Mortar::Auth.api).post_job_existing_cluster("myproject", "my_script", is_a(String), "0.9", cluster_id, :parameters => [], :notify_on_job_finish => false, :is_control_script=>false) {Excon::Response.new(:body => {"job_id" => job_id, "web_job_url" => job_url})}
...
STDOUT
      ...
      it "runs a job by default on the largest existing running cluster" do
        with_git_initialized_project do |p|
          ...
          mock(Mortar::Auth.api).post_job_existing_cluster("myproject", "my_script", is_a(String), "0.9", large_cluster_id, 
            :parameters => [], 
            :notify_on_job_finish => true,
            :is_control_script=>false) {Excon::Response.new(:body => {"job_id" => job_id, "web_job_url" => job_url})}
