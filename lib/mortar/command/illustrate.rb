...
class Mortar::Command::Illustrate < Mortar::Command::Base
  ...
  def index
    ...
    action("Starting illustrate") do
      illustrate_id = api.post_illustrate(project.name, pigscript.name, alias_name, skip_pruning, git_ref, pig_version_str, :parameters => pig_parameters).body["illustrate_id"]
    end
